...
my_dollar <- dollar_format(negative_parens = FALSE)
```
...
renderPlotly({
  ...
  dataset <-
    ...
    mutate(Stipend = dollar(stipend),
           `Insurance Included` = if_else(str_detect(category, "insured"), "Yes", "No"),
           Students = count,
  ...
  p <- g +
    ...
    scale_x_continuous(breaks = seq(24000, 34000, 1000), labels = label_dollar()) +
...
renderValueBox({
  ...
  valueBox(my_dollar(difference),
           caption = case_when(
             difference == 0 ~ "Budget",
             difference > 0 ~ "Budget Increase Required",
             difference < 0 ~ "Budget Surplus"
           ),
           icon = "fa-dollar-sign",
           color = case_when(
...
renderValueBox({
  ...
  valueBox(my_dollar(per_student),
